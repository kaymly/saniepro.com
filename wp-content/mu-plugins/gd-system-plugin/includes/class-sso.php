...
final class SSO {
	...
	/**
	 * Excluded classes hooked into 'login_form' or 'login_enqueue_scripts'.
	 *
	 * @var array
	 */
	...
	public function __construct( API_Interface $api ) {
		...
		/**
		 * Excluded plugin classes.
		 *
		 * @var array
		 */
		$this->excluded_classes = (array) apply_filters(
				'wpaas_gd_login_enqueue_scripts_plugin_classes',
				array(
				'Limit_Login_Attempts',
				'Wpsec\twofa\Core\TwoFactorAuthCore',
				'Wpsec\captcha\handlers\LoginFormEventHandler',
			)
		);
	...
	public function init() {
		...
		$uri      = sanitize_url( filter_input( INPUT_SERVER, 'REQUEST_URI' ) );
		...
		if ( is_int( $user_id ) ) {
			...
			if ( $hash && $this->api->is_valid_sso_hash( $hash ) ) {
                ...
                /** Logg user action */
                $GLOBALS['wpaas_activity_logger']->log_sp_action($user_id, 'SSO login success' );
        ...
        /** Logg user action */
        $GLOBALS['wpaas_activity_logger']->log_sp_action($user_id, 'SSO login fail' );
	...
	public function login_init() {
		...
		if ( ! $enabled || ! Plugin::is_gd() || ! defined( 'GD_ACCOUNT_UID' ) || ! GD_ACCOUNT_UID || ( isset( $_GET['wpaas-standard-login'] ) && (bool) $_GET['wpaas-standard-login'] ) || $this->is_plugin_hooked( 'login_form' ) || $this->is_plugin_hooked( 'login_enqueue_scripts' ) ) {
	...
	/**
	 * Determine if a plugin is hooked into 'login_enqueue_scripts' and customizing the login screen.
	 *
	 * @param string $action The action to check against.
	 *
	 * @return boolean True when a plugin is hooked into $action, else false.
	 */
	private function is_plugin_hooked( $action ) {

		global $wp_filter;

		if ( empty( $action ) || ! array_key_exists( $action, $wp_filter ) ) {

			return false;

		}

		$hooked_actions = array();
		$data           = $wp_filter[ $action ]->callbacks[ key( $wp_filter[ $action ]->callbacks ) ];
		...
		foreach ( $data as $info ) {
			...
			// Closure.
			if ( is_object( $info['function'] ) ) {

				$hooked_actions[] = get_class( $info['function'] );
			...
			if ( is_array( $info['function'] ) ) {
				...
				foreach ( $info['function'] as $hook ) {

					if ( is_object( $hook ) ) {

						$hooked_actions[] = get_class( $hook );
		...
		if ( ! empty( $hooked_actions ) ) {

			$hooked_actions = array_diff( $hooked_actions, $this->excluded_classes );

		}

		return ! empty( array_filter( $hooked_actions ) );
