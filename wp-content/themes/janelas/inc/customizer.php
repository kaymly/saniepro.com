...
function themesflat_customize_register( $wp_customize ) {
   ...
   //___FOOTER___//
    ...
    //___LAYOUT___//
    $wp_customize->get_section( 'background_image' )->title = esc_html__('Layout Style', 'janelas');
...
function themesflat_sanitize_bg_size( $input ) {
    $valid = array(
        'cover'     => esc_html__('Cover', 'janelas'),
        'contain'   => esc_html__('Contain', 'janelas'),
    );
...
function themesflat_sanitize_blog( $input ) {
    $valid = array(
        ...
        'fullwidth'  => esc_html__( 'Full width (no sidebar)', 'janelas' )
...
function themesflat_sanitize_pagination ( $input ) {
    $valid = array(
        ...
        'page_numeric' => esc_html__('Pager & Numeric', 'janelas')                
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_related_post ( $input ) {
    $valid = array(
        'simple_list' => esc_html__('Simple List', 'janelas'),
        'grid' => esc_html__('Grid', 'janelas')        
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_fw( $input ) {
    $valid = array(
        '0' => esc_html__('footer_default', 'janelas'),
        '1' => esc_html__('One', 'janelas'),
        '2' => esc_html__('Two', 'janelas'),
        '3' => esc_html__('Three', 'janelas'),
        '4' => esc_html__('Four', 'janelas')
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_related_portfolio( $input ) {
    $valid = array(
        'grid'                 => esc_html__( 'Grid', 'janelas' ),
        'grid_masonry'         => esc_html__( 'Grid Masonry', 'janelas' ),
        'grid_nomargin'        => esc_html__( 'Grid Masonry No Margin', 'janelas' ),
        'carosuel'             => esc_html__( 'Carosuel', 'janelas' ),
        'carosuel_nomargin'    => esc_html__( 'Carosuel No Margin', 'janelas' )       
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_pagination( $input ) {
    $valid = array(
        'page_numeric'         => esc_html__( 'Pager & Numeric', 'janelas' ),
        'load_more'         => esc_html__( 'Load More', 'janelas' )     
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_order( $input ) {
    $valid = array(
        ...
        'id'            => esc_html__( 'Id', 'janelas' ),
        'author'        => esc_html__( 'Author', 'janelas' ),
        'title'         => esc_html__( 'Title', 'janelas' ),
        'modified'      => esc_html__( 'Modified', 'janelas' ),
        'comment_count' => esc_html__( 'Comment Count', 'janelas' ),
        'menu_order'    => esc_html__( 'Menu Order', 'janelas' )     
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_order_direction( $input ) {
    $valid = array(
        'DESC' => esc_html__( 'Descending', 'janelas' ),
        'ASC'  => esc_html__( 'Assending', 'janelas' )       
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_grid_portfolio( $input ) {
    $valid = array(
        'portfolio-two-columns'     => esc_html__( '2 Columns', 'janelas' ),
        'portfolio-three-columns'     => esc_html__( '3 Columns', 'janelas' ),
        'portfolio-four-columns'     => esc_html__( '4 Columns', 'janelas' ),
        'portfolio-five-columns'     => esc_html__( '5 Columns', 'janelas' )
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_grid_portfolio_related( $input ) {
    $valid = array(
        'portfolio-one-columns'     => esc_html__( '1 Columns', 'janelas' ),
        'portfolio-two-columns'     => esc_html__( '2 Columns', 'janelas' ),
        'portfolio-three-columns'     => esc_html__( '3 Columns', 'janelas' ),
        'portfolio-four-columns'     => esc_html__( '4 Columns', 'janelas' ),
        'portfolio-five-columns'     => esc_html__( '5 Columns', 'janelas' )
...
function themesflat_sanitize_layout_product( $input ) {
    $valid = array(        
        'fullwidth'         => esc_html__( 'No Sidebar', 'janelas' ),
        'sidebar-right'           => esc_html__( 'Sidebar Right', 'janelas' ),
        'sidebar-left'         => esc_html__( 'Sidebar Left', 'janelas' )
    );
    if ( array_key_exists( $input, $valid ) ) {
