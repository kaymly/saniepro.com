...
class ExtensionsController extends AbstractController
{
    ...
    protected function prepareItem(AbstractExtension $extension, bool $includeVersions = false) : array
    {
        ...
        return $this->maybeOverrideExtensionData($extension, $extensionData);
    }

    /**
     * May overrides extension data from configuration.
     *
     * @param AbstractExtension $extension
     * @param array<string, mixed> $extensionData
     * @return array<string, mixed>
     */
    protected function maybeOverrideExtensionData(AbstractExtension $extension, array $extensionData) : array
    {
        if ($overrideData = Configuration::get("mwc_extensions.overrides.{$extension->getSlug()}")) {
            try {
                return ArrayHelper::combineRecursive($extensionData, $overrideData);
    ...
    protected function getExcludedExtensions() : array
    {
        ...
        if ($platformRepository->getGoDaddyCustomer()->getFederationPartnerId() === 'WORLDPAY') {
            $excludedSlugs = ArrayHelper::combine(
                $excludedSlugs,
                array_keys(TypeHelper::array(Configuration::get('mwc_extensions.categories'), []), 'Payments')
            );
        }
        ...
        foreach ($exclusionRules as $slug => $rules) {
            ...
            $countriesNotIn = TypeHelper::array(ArrayHelper::get($rules, 'countriesNotIn'), []);
            ...
            if ($countriesNotIn && ArrayHelper::contains($countriesNotIn, WooCommerceRepository::getBaseCountry())) {
                continue;
            }
            ...
            if ($plans && ! ArrayHelper::contains($plans, $platformRepository->getPlan()->getName())) {
                continue;
            }

            if (null !== $reseller && $reseller !== $platformRepository->isReseller()) {
                continue;
            }
    ...
    public function updateItem($request)
    {
        ...
        try {
            $extension = $this->getManagedExtension(SanitizationHelper::input($request->get_param('slug')));
