...
final class Debug_Mode {
	...
	public static function is_start() {
		...
		$action   = sanitize_text_field( filter_input( INPUT_GET, 'GD_COMMAND' ) );
		$sso_hash = sanitize_text_field( filter_input( INPUT_GET, 'SSO_HASH' ) );
	...
	private function is_exit() {
		...
		return ( 'DEBUG_EXIT' === sanitize_text_field( filter_input( INPUT_GET, 'GD_COMMAND' ) ) );
	...
	private function update_session() {
		...
		$active_theme = sanitize_key( filter_input( INPUT_POST, 'wpaas-debug-mode-theme' ) );

		if ( 'installed' === $active_theme ) {

			$active_theme = sanitize_text_field( filter_input( INPUT_POST, 'wpaas-debug-mode-installed-theme' ) );
