...
use GoDaddy\WordPress\MWC\Core\WooCommerce\Exceptions\FailedVoidOrderException;
...
abstract class AbstractPaymentGateway extends WC_Payment_Gateway
{
    ...
    public function getTransactionForPayment(WC_Order $order) : PaymentTransaction
    {
        ...
        // If the order was created via other means than regular checkout (i.e. Apple Pay), set it as the source on
        // transaction. This provides context for payment processing later on.
        if ($source = $order->get_created_via()) {
     ...
     *
     * @param mixed $gatewayTitle
     * @param mixed $paymentGatewayId
     * @return mixed
     * @throws Exception
     */
    public function filterGatewayTitleBySource($gatewayTitle, $paymentGatewayId)
    {
        ...
        $order = OrdersRepository::get((int) $orderId);
        ...
        // If the order has been paid for with a wallet gateway (Apple Pay, Google Pay), then the $paymentGatewayId will
        // be 'poynt' (not the wallet gateway ID). Instead, the 'get_created_via' meta value will hold the wallet ID, ie 'mwc_payments_apple_pay'
        foreach (CorePaymentGateways::getWalletGateways() as $id => $walletGateway) {
            if ($order->get_created_via() === 'mwc_payments_'.StringHelper::snakeCase($walletGateway::getWalletId())) {
                return $walletGateway->getOrderPaymentMethodTitle((int) $orderId);
            }
        }
     ...
     * @param WC_Order $order
     * @param AbstractTransaction $transaction
     * @throws BaseException|FailedRemoteParentIdException
     */
    protected function maybeSetRemoteParentId(WC_Order $order, AbstractTransaction $transaction)
    {
        try {
            ...
            throw new FailedRemoteParentIdException($exception->getMessage(), $exception);
        }
    }
     ...
     */
    public function processPaymentResult(PaymentTransaction $transaction, WC_Order $wooOrder) : WC_Order
    {
        ...
        if ($status instanceof DeclinedTransactionStatus) {
            ...
            throw new FailedTransactionException('The transaction failed.');
        }
    ...
    public function process_refund($order_id, $amount = null, $reason = null)
    {
        try {
            ...
            if (! $transaction->getStatus() instanceof ApprovedTransactionStatus) {
                throw new FailedTransactionException($transaction->getResultMessage());
            }

            return true;
        } catch (Exception $exception) {
            return new WP_Error($exception->getCode() ?: 400, $exception->getMessage());
        }
    }
    ...
    protected function filterVoidedStatus()
    {
        Register::filter()
            ...
            ->setHandler([$this, 'getVoidedStatusValue'])
            ->execute();
    }

    /**
     * Gets the voided status value for a filter callback.
     *
     * @internal
     *
     * @return string
     */
    public function getVoidedStatusValue() : string
    {
        return 'cancelled';
    }
    ...
    public function processVoid(VoidTransaction $transaction) : VoidTransaction
    {
        if ($transaction->getTotalAmount() && $transaction->getOrder() && $transaction->getTotalAmount()->getAmount() !== $transaction->getOrder()->getTotalAmount()->getAmount()) {
            throw new FailedVoidOrderException('Oops, you cannot partially void this order. Please use the full order amount.');
        }
    ...
    private function findPaymentMethodFromToken($customerId = 0) : AbstractPaymentMethod
    {
        ...
        if ($paymentTokenId) {
            ...
            if ((int) $paymentMethod->getCustomerId() !== (int) $customerId) {
                throw new InvalidTransactionException('Invalid payment method ID');
            }
        } else {
    ...
    protected function updateConfigurationFromSettings($configurations = null)
    {
        ...
        foreach ($configurations as $configurationKey => $wooKey) {
            ...
            try {
                ...
                throw new PaymentsProviderSettingsException("Payments configuration {$configurationKey} could not be set with value {$settingValue}", $exception);
            }
        }
    }
}
